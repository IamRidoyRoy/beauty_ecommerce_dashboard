...
export const shippingApi=baseApi.injectEndpoints({endpoints:b=>({
  ...
  courierDispatchOrders:b.query<any,Record<string,any>|void>({query:p=>({url:'/admin/shipments/courier-orders/',params:p||{}}),transformResponse:tr,providesTags:['Orders','Shipments']}),
  submitCourierOrders:b.mutation<{submitted:any[];failed:any[];submitted_count:number;failed_count:number},{order_ids:number[];provider:string;options?:Record<string,any>}>({query:body=>({url:'/admin/shipments/submit-orders/',method:'POST',body}),transformResponse:tr,invalidatesTags:['Shipments','Orders','Dashboard','Reports']}),
...
export const {useShippingMethodsQuery,useCreateShippingMethodMutation,useUpdateShippingMethodMutation,useShipmentsQuery,useCourierDispatchOrdersQuery,useSubmitCourierOrdersMutation,useAvailableCouriersQuery,useBookShipmentMutation,useTrackShipmentMutation,useCancelShipmentMutation,useRequestShipmentReturnMutation,useShipmentHistoryQuery,useLazyShipmentHistoryQuery,useCourierConfigsQuery,useUpdateCourierConfigMutation,useTestCourierConnectionMutation}=shippingApi
