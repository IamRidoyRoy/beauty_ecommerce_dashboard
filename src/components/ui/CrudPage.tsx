...
export type CrudField={key:string;label:string;type?:'text'|'number'|'textarea'|'checkbox'|'select';options?:Array<{value:any;label:string}>;placeholder?:string;required?:boolean;helpText?:string;defaultValue?:any}

type Props={title:string;description?:string;data:any;isLoading:boolean;isError:boolean;error?:any;refetch:()=>void;fields:CrudField[];createItem:(body:any)=>Promise<any>;updateItem:(id:number,body:any)=>Promise<any>;deleteItem?:(id:number)=>Promise<any>;renderRow?:(item:any)=>ReactNode}

export function CrudPage({title,description,data,isLoading,isError,error,refetch,fields,createItem,updateItem,deleteItem,renderRow}:Props){
  ...
  const begin=(item?:any)=>{
    ...
    setErrors({})
    // Only hydrate fields that actually exist in this form. API-only fields such as
    // category.image/seo must never leak into a PATCH payload as null values.
    setForm(Object.fromEntries(fields.map(f=>{
      const fallback=f.defaultValue!==undefined?f.defaultValue:(f.type==='checkbox'?false:'')
      return [f.key,item ? (item[f.key] ?? fallback) : fallback]
    })))
    setOpen(true)
  }
  ...
  const save=async()=>{
    ...
    setBusy(true)
    try{
      // Whitelist the form schema when submitting. This prevents read-only or
      // hidden response fields from being sent back during generic CRUD edits.
