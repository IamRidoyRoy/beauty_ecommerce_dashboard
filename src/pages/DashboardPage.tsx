...
export default function DashboardPage(){
  ...
  return <>
    <PageHeader title="Dashboard" description="Placed orders count in revenue immediately; cancelled and completed returned/refunded orders are excluded." actions={<>{can(user,'orders_write')&&<Link className="btn-brand px-5 py-3 text-[15px] shadow-lg shadow-pink-200/70 ring-1 ring-pink-800/10 hover:-translate-y-0.5 hover:shadow-xl" to="/sales/orders/new"><Plus size={16}/>Create Order</Link>}<button className="btn-secondary" onClick={()=>{d.refetch();sales.refetch();products.refetch();orders.refetch();stock.refetch()}}>Refresh</button></>}/>
