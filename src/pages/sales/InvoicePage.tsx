...
export default function InvoicePage(){
  ...
  useEffect(()=>{if(q.data&&params.get('print')==='1'&&!printed.current){printed.current=true;window.setTimeout(()=>window.print(),300)}},[q.data,params])
  ...
  const couponEntry=(o.promotion_snapshot||[]).find((x:any)=>x?.type==='coupon'||(o.coupon_code_snapshot&&x?.code===o.coupon_code_snapshot))
  return <div className="print-page mx-auto max-w-5xl rounded-2xl border border-zinc-200 bg-white p-7 shadow-sm sm:p-10">
    ...
    <div className="overflow-x-auto"><table className="table invoice-table"><thead><tr><th>Product</th><th>SKU / Variant</th><th>Qty</th><th>Unit</th><th>Discount</th><th>Total</th></tr></thead><tbody>{o.items?.map((i:any)=><tr key={i.id}><td><span className="font-medium">{i.product_name_snapshot}</span></td><td><div>{i.sku_snapshot}</div><div className="text-xs text-zinc-400">{Object.entries(i.variant_snapshot||{}).map(([k,v])=>`${k}: ${v}`).join(' · ')}</div></td><td>{i.quantity}</td><td>{money(i.unit_price)}</td><td>{money(i.discount)}</td><td className="font-semibold">{money(i.total)}</td></tr>)}</tbody></table></div>
    <div className="ml-auto mt-8 w-full max-w-sm space-y-2 text-sm"><div className="flex justify-between"><span className="text-zinc-500">Subtotal</span><span>{money(o.subtotal)}</span></div>{o.coupon_code_snapshot&&<div className="flex justify-between"><span className="text-zinc-500">Coupon ({o.coupon_code_snapshot})</span><span>{couponEntry?.free_shipping?'Free shipping':`-${money(couponEntry?.discount??o.discount)}`}</span></div>}<div className="flex justify-between"><span className="text-zinc-500">Shipping</span><span>{money(o.shipping_charge)}</span></div><div className="flex justify-between"><span className="text-zinc-500">Tax</span><span>{money(o.tax)}</span></div><div className="flex justify-between"><span className="text-zinc-500">Total Discount</span><span>-{money(o.discount)}</span></div><div className="flex justify-between border-t border-zinc-200 pt-3 text-lg font-bold"><span>Grand Total</span><span>{money(o.total)}</span></div></div>
