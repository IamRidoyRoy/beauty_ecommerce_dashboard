...
function ModuleAccess({modules,allowed,selected,onChange}:{modules:StaffAccessModule[];allowed:string[];selected:string[];onChange:(x:string[])=>void}){
 const usable=modules.filter(m=>allowed.includes(m.key));
 const groups=useMemo(()=>Array.from(new Set(usable.map(m=>m.group))),[usable]);
 const ordered=(values:Iterable<string>)=>{const set=new Set(values);let changed=true;while(changed){changed=false;for(const m of usable)if(set.has(m.key))for(const req of m.requires||[])if(allowed.includes(req)&&!set.has(req)){set.add(req);changed=true}}return usable.filter(m=>set.has(m.key)).map(m=>m.key)}
 const toggle=(key:string)=>{const next=new Set(selected);if(next.has(key)){next.delete(key);for(const m of usable)if((m.requires||[]).includes(key))next.delete(m.key)}else{next.add(key);for(const req of usable.find(m=>m.key===key)?.requires||[])next.add(req)}onChange(ordered(next))}
 const selectGroup=(group:string)=>onChange(ordered([...selected,...usable.filter(m=>m.group===group).map(m=>m.key)]))
 const clearGroup=(group:string)=>{const removing=new Set(usable.filter(m=>m.group===group).map(m=>m.key));const keep=selected.filter(key=>!removing.has(key));onChange(ordered(keep))}
 return <div className="rounded-2xl border border-zinc-200 bg-zinc-50/60 p-4">
  <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="font-semibold">Detailed dashboard access</div><p className="mt-1 max-w-2xl text-xs leading-5 text-zinc-500">Choose page-by-page access. For example, Coupons can be enabled while Promotions, Campaigns and Homepage Content remain disabled. The same permission is enforced by the backend API.</p></div><div className="flex gap-2"><button type="button" className="btn-secondary py-2 text-xs" onClick={()=>onChange(ordered(usable.map(x=>x.key)))}>Select all</button><button type="button" className="btn-secondary py-2 text-xs" onClick={()=>onChange([])}>Clear all</button></div></div>
  <div className="mt-5 space-y-5">{groups.map(group=>{const items=usable.filter(m=>m.group===group);const count=items.filter(m=>selected.includes(m.key)).length;return <section key={group} className="rounded-xl border border-zinc-200 bg-white p-3 sm:p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><div className="text-xs font-bold uppercase tracking-[.14em] text-zinc-500">{group}</div><div className="mt-0.5 text-[11px] text-zinc-400">{count} of {items.length} selected</div></div><div className="flex gap-2"><button type="button" className="rounded-lg border border-zinc-200 px-2.5 py-1.5 text-[11px] font-semibold hover:bg-zinc-50" onClick={()=>selectGroup(group)}>Select group</button><button type="button" className="rounded-lg border border-zinc-200 px-2.5 py-1.5 text-[11px] font-semibold hover:bg-zinc-50" onClick={()=>clearGroup(group)}>Clear</button></div></div><div className="grid gap-2 sm:grid-cols-2">{items.map(m=>{const checked=selected.includes(m.key);const reqLabels=(m.requires||[]).map(k=>usable.find(x=>x.key===k)?.label||k);return <button type="button" key={m.key} onClick={()=>toggle(m.key)} className={`flex items-start gap-3 rounded-xl border p-3 text-left transition ${checked?'border-pink-300 bg-pink-50':'border-zinc-200 bg-white hover:border-zinc-300'}`}><span className={`mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded border ${checked?'border-pink-700 bg-pink-700 text-white':'border-zinc-300 bg-white'}`}>{checked&&<Check size={13}/>}</span><span className="min-w-0"><b className="text-sm">{m.label}</b><span className="mt-0.5 block text-xs leading-4 text-zinc-500">{m.description}</span>{reqLabels.length>0&&<span className="mt-1 block text-[10px] font-medium text-amber-700">Requires: {reqLabels.join(', ')}</span>}</span></button>})}</div></section>})}</div>
 </div>
}
